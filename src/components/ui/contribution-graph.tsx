...
const fillHoles = (activities: Activity[]): Activity[] => {
  ...
  const calendar = new Map(sortedActivities.map((activity) => [activity.date, activity]));
...
const getMonthLabels = (
  ...
  weeks
    .reduce<MonthLabel[]>((labels, week, weekIndex) => {
      ...
      if (!previousLabel || previousLabel.label !== month) {
    ...
    .filter(({ weekIndex }, index, labels) => {
      ...
      if (index === 0) {
        return Boolean(labels[1] && labels[1].weekIndex - weekIndex >= minWeeks);
      }
...
export const ContributionGraph = ({
  ...
  return (
    ...
    >
      <div
        className={cn("flex w-full max-w-full flex-col gap-3", className)}
...
export const ContributionGraphBlock = ({
  ...
  if (activity.level < 0 || activity.level > maxLevel) {
    throw new RangeError(
      `Provided activity level ${activity.level} for ${activity.date} is out of range.`
    );
  }
...
export const ContributionGraphFooter = ({
  ...
  <div
    className={cn(
      "flex flex-wrap items-center gap-2 whitespace-nowrap text-sm sm:gap-x-4",
      className
    )}
    {...props}
  />
